...
local Tabs = {Home=Window:AddTab({Title="Thông Tin"}),Main=Window:AddTab({Title="Chung"}),Sea=Window:AddTab({Title="Sự Kiện"}),ITM=Window:AddTab({Title="Vật Phẩm"}),Setting=Window:AddTab({Title="Cài Đặt"}),Status=Window:AddTab({Title="Máy Chủ"}),Stats=Window:AddTab({Title="Chỉ Số"}),Player=Window:AddTab({Title="Người Chơi"}),Teleport=Window:AddTab({Title="Dịch Chuyển"}),Visual=Window:AddTab({Title="Giả"}),Fruit=Window:AddTab({Title="Trái"}),Raid=Window:AddTab({Title="Tập Kích"}),Race=Window:AddTab({Title="Tộc"}),Shop=Window:AddTab({Title="Cửa Hàng"}),Misc=Window:AddTab({Title="Khác"})};
...
spawn(function()
    while wait() do
        local Mode = getgenv().FarmMode
        local AutoFarm = getgenv().AutoFarm
        local Player = game.Players.LocalPlayer
        local HRP = Player.Character and Player.Character:FindFirstChild("HumanoidRootPart")
        if not Mode or not AutoFarm or not HRP then continue end
        pcall(function()
